...
export async function processPart2() {
  ...
  const timeAllowed = lines[0].replaceAll(/\s+/g,"").split(":");
  console.log(timeAllowed)
  console.log("and")
  const distanceRecord = lines[1].replaceAll(/\s+/g,"").split(":");
  ...
  try {
    ...
    for (let racesRun = 1; racesRun < timeAllowed.length; racesRun++) {
      ...
      console.log("I found "+ waysFound+" ways to beat the "+ timeAllowed[racesRun]+" record")
